...
public class EnderChest implements InventoryProvider {
    private UUID uuid;
    ...
    public EnderChest(UUID uuid) {
        this.uuid = uuid;
        for (ArrayList<ItemStack> list : TaktiePlugin.getPlugin().getDataManager().getInitEnderChest().getEnderChestList()) {
            enderChestList.add(new ArrayList<>(list));
        }
    }
    ...
    public SmartInventory getInv(){
        return SmartInventory.builder()
                ...
                .provider(this)
                .size(4, 9)
                .title("§7§l" + Bukkit.getOfflinePlayer(uuid).getName() + "'s EnderChest")
                .id("EnderChest_" + Bukkit.getOfflinePlayer(uuid).getName())
                .update(false)
    ...
    private InventoryListener<InventoryCloseEvent> getCloseListener() {
        return new InventoryListener<>(InventoryCloseEvent.class, e -> {
            ...
            TaktiePlugin.getPlugin().getDataManager().saveEnderChest(this);
        });
    }

    public void addPage() {
        ArrayList<ItemStack> page = new ArrayList<>(); for (int i = 0; i < 27; i++) page.add(new ItemStack(Material.AIR));
        enderChestList.add(page);
    }

    public void removePage() {
        enderChestList.remove(enderChestList.size() - 1);
    }
